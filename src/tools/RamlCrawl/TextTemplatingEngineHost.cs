...
namespace RamlCrawl
{
    ...
    public class TextTemplatingEngineHost : ITextTemplatingEngineHost, ITextTemplatingSessionHost
    {
        ...
        public string ResolveAssemblyReference(string assemblyReference)
        {
            ...
            var assemblyInBinDir = assemblyReference.Replace(
                "$(binDir)",
                Path.GetDirectoryName(GetType().Assembly.Location) + Path.DirectorySeparatorChar);
            if (File.Exists(assemblyInBinDir))
            {
                return assemblyInBinDir;
            }

            var assemblyName = assemblyReference.EndsWith(".dll", StringComparison.OrdinalIgnoreCase)
                ? Path.GetFileNameWithoutExtension(assemblyReference)
                : Path.GetFileName(assemblyReference);
        ...
        public string ResolveParameterValue(string directiveId, string processorName, string parameterName)
        {
            if (directiveId == null)
            {
                throw new ArgumentNullException(nameof(directiveId));
            }
        ...
        {
            return Session ?? (Session = new TextTemplatingSession());
        }
